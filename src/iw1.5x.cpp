...
void custom_Com_Init(char *commandLine)
{
    ...
    *(int*)&Com_Init = hook_Com_Init->from;
    ...
    // Register
    Cvar_Get("iw1.5x", "1", CVAR_SERVERINFO | CVAR_ROM);
...
void custom_SV_PacketEvent(netadr_t from, msg_t *msg)
{
    ...
    int *unknown_var = (int*)0x80EE80C; // Maybe related to scrVmGlob.loading
...
void custom_SV_ExecuteClientCommand(client_t *cl, const char *s, qboolean clientOK)
{
    ...
    ((void(*)(int))0x80CB8EE)(1); // XAnimSetUser, seems related to scrAnimGlob 
...
void CrashLogger(int sig)
{
    ...
    fs_lnxded_path.replace_filename("iw1.5x_crash.log");
...
class iw1x
{
    ...
    iw1x()
    {
       ...
       */
        ...
        hook_Com_Init->hook();
        hook_SV_SpawnServer = new cHook(0x8090D0F, (int)custom_SV_SpawnServer);
        hook_SV_SpawnServer->hook();
        hook_SV_BeginDownload_f = new cHook(0x808B456, (int)custom_SV_BeginDownload_f);
        hook_SV_BeginDownload_f->hook();
    ...
    ~iw1x()
    {
        printf("iw1.5x destructor called\n");
        system("stty sane");
    }
};
